...
function App() {
	const data: string[] = ['Избранные', 'Акции', 'Новинки', 'Fast Food', 'Азиатская', 'Кофейня', 'Пиццерия', 'Бургерная']

	const mainBtn: string = 'bg-[#ececee] hover:bg-[#e0e0e4] py-[14px] px-[16px]  font-semibold rounded-[12px] cursor-pointer  transition-all'
	...
	return (
		<>
			...
			<main className='container  pt-[128px] pb-[120px] flex flex-col gap-[56px]'>
				<section>
					<Swiper modules={[Navigation]} spaceBetween={14} slidesPerView={6} navigation={true} className=' flex gap-[12px]'>
						{[...Array(10)].map(item => {
							return (
								<SwiperSlide className='scrollerImg' style={{ width: 'min-content', marginRight: '14px' }}>
				...
				<section>
					...
					<div className=''>
						<Swiper modules={[Navigation]} spaceBetween={14} slidesPerView={5} navigation={true} className='flex gap-[12px] relative'>
							{[...Array(15)].map((item, i) => (
								<SwiperSlide key={i} className='scrollerLittleCard' style={{ width: 'min-content', marginRight: '14px' }}>
									<LittleCard />
								</SwiperSlide>
							))}
						</Swiper>
